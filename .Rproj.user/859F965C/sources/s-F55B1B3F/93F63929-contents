...
person_df1 <- read.csv(file='data/tidynomicon/person.csv',sep=',')
str(person_df1)
...
person_df2 <- read.csv(file='data/tidynomicon/person.csv',stringsAsFactors = FALSE,sep=',')
str(person_df2)

## Read the file `data/scores.csv` to `scores_df`
## Display summary statistics using the `summary()` function
scores_df <- read.csv(file='data/scores.csv',sep=',')
summary(scores_df)
...
person_df <- dbGetQuery(db, "SELECT * FROM PERSON;", stringAsfactors = FALSE)
head(person_df)
