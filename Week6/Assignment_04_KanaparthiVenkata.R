...
covid_df <- read.csv("data/nytimes/covid-19-data/us-states.csv", stringsAsFactors = FALSE)
...
ggplot(data=florida_df, aes(x=date,y=cases,group=1)) + geom_line() + labs(x="Date", y="Cases") 

## Add lines for New York and California to the plot
ggplot(data=florida_df, aes(x=date, group=1)) +
  geom_line(aes(y = cases)) +
  geom_line(data=ny_df, aes(y = cases)) +
  geom_line(data=california_df, aes(y = cases), na.rm=TRUE)  + labs(x="Date", y="Cases") 
